...
A command-line static analysis tool that parses 64-bit ELF binaries from scratch (no `libelf`/`libbfd`) and flags structural and behavioral indicators commonly associated with malware, packers, and memory-corruption exploits.
...
Requires a C++23 compiler and CMake 3.21+.
...
`tests/` contains real sample binaries used to validate detection logic:

- `tests/benign/` — legitimate Linux binaries (e.g. `ls`, `sha256sum`) that should produce no alerts
- `tests/malware/` — a binary exhibiting packer-like and W^X characteristics used to validate the heuristics

```
./build/elfpeek tests/benign/x86__64__lsb__unix-system-sha256sum
./build/elfpeek tests/malware/x86__64__lsb__unix-system-server
```
...
Built by [Isaiah/ike](https://github.com/ikeWasHere) as a systems/security-focused portfolio project. Feedback and issues welcome.
